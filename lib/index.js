...
/**
 * Tests given a series of points that sequential points are closer to
 * each other than non sequential points. Subdivides the list of points
 * recursively testing smaller chunks of the list of points. Solves in
 * Polynomial time!
 *
 * @param points
 * @returns {boolean} whether or not all mid points are closer
 */
function validateLandmarksMidPointsCloser(points) {
    if (points.length < 3) {
        return true;
    }
    const start = points[0];
    const end = points[points.length - 1];
    const pivot = points.length >> 1;
    ...
    for (let i = 0; i < points.length - pivot; ++i) {
        if (!validateLandmarksMidPointsCloser(points.slice(i, i + pivot + 1))) {
            return false;
        }
    }
    return true;
...
function assignPoints(points, distance) {
    ...
    const scored = perms.map(points => {
        return ({
            points,
            score: validateLandmarksMidPointsCloser(points) * 1 + validateLandmarksMinSeparation(points, distance) * 1
        })
    });
    shuffle(scored);
